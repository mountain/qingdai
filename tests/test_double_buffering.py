...
def test_world_mini_state_integration():
    # Minimal smoke: ensure WorldState has DBA mini and swap_all flips it
    from pygcm.world import QingdaiWorld

    w = QingdaiWorld.create_default()
    # current_state has mini.tick DBA allocated in __init__
    assert w.current_state.mini is not None
    dba = w.current_state.mini.tick
    # Write a value into write and swap via state's swap_all()
    dba[...] = 42.0
    w.current_state.swap_all()
    np.testing.assert_allclose(dba.read, 42.0)
